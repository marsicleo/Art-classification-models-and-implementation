...
RUN_NAME = "Tate_Collection_MobileNetV3_v3"
IMAGE_SIZE = (224, 224)
BATCH_SIZE = 8 
EPOCHS_PHASE1 = 10 
EPOCHS_PHASE2 = 55
...
def build_model(num_classes, unfreeze_base=False, load_weights_path=None):
    ...
    weights = 'imagenet' if not load_weights_path else None
    ...
    if load_weights_path:
        mobilenet_model.load_weights(load_weights_path, by_name=True, skip_mismatch=True)    
    x = mobilenet_model(x) 
    
    def eca_block(inputs):
...
with mlflow.start_run(run_name=RUN_NAME):
    try:
        ...
        if 'train_df' not in locals() and 'train_df' not in globals():
            logger.warning("Training DataFrame not available for class weight calculation. Iterating dataset (may be slow).")
            all_train_labels = []
        ...
        backup = tf.keras.callbacks.BackupAndRestore(backup_dir=RUN_OUTPUT_DIR)

        callbacks_list = [early_stopping, lr_scheduler, terminate_nan, backup]
        ...
        phase2_callbacks = [
            ...
            tf.keras.callbacks.BackupAndRestore(backup_dir=RUN_OUTPUT_DIR)
        ]        
